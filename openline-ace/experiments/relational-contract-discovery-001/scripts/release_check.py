#!/usr/bin/env python3
"""Orthogonal release checks for the ACE RCDL experiment."""
...
def main() -> int:
    ...
    run([sys.executable, "-m", "compileall", "-q", "rcdl", "tests", "scripts"], env=env)
    results["compileall"] = "PASS"
    test_output = run(
        [sys.executable, "-m", "unittest", "discover", "-s", "tests", "-v"], env=env
    )
    results["unit_tests"] = {
        "status": "PASS",
        "test_count": test_output.count(" ... ok"),
    }
    reference_output = run([sys.executable, "-m", "rcdl", "verify-reference"], env=env)
    results["official_reference"] = json.loads(reference_output)
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        for output in (first, second):
            ...
            run(
                [
                    ...
                    "rcdl",
                    "verify-manifest",
                    str(output / "contract-manifest.json"),
                ],
                env=env,
            )
            run(
                [
                    ...
                    "rcdl",
                    "verify-projection",
                    str(output / "contract-projection.json"),
                ],
                env=env,
            )
        ...
        isolated = temp / "isolated-experiment"
        shutil.copytree(
            ROOT,
            isolated,
