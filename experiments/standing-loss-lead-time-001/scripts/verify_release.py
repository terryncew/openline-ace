...
checks = {
    ...
    "hashes": all(rel in actual and actual[rel]["sha256"] == item["sha256"] for rel, item in expected.items()),
    "sizes": all(rel in actual and actual[rel]["size"] == item["size"] for rel, item in expected.items()),
    "policy_authority_none": manifest.get("policy_authority") == "NONE",
    "runtime_permission_none": manifest.get("runtime_permission") == "NONE",
}
out = {"schema": "openline.ace.sld001.release-verification.v1", "verified": all(checks.values()), "checks": checks, "file_count": len(actual)}
