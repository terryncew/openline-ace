"""Command-line interface for RCDL-003."""
...
from .contracts import frozen_clauses
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="rcdl003")
    ...
    commands.add_parser("show-clauses")
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "run":
            result = run_experiment(args.output, trials=args.trials, force=args.force)
        ...
        elif args.command == "show-clauses":
            result = {
                "schema": "rcdl.frozen-clause-list/0.3",
                "clauses": [clause.to_dict() for clause in frozen_clauses()],
            }
